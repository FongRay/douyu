...
int douyu_danmu(int sockfd, const char* username,
    ...
    MsgInfo msg;
    int ct_len = _snprintf(msg.content, sizeof(msg.content),
    ...
    send(sockfd, sendBuf, msg.len+sizeof(msg.len), 0);
    recv(sockfd, sendBuf, sizeof(msg), 0);
    printf("recv:%s\n", msg.content);

    ct_len = _snprintf(msg.content, sizeof(msg.content),
    ...
    int ret = 0;
    while (true) {
        ret = recv(sockfd, sendBuf, sizeof(msg), 0);
        if (process_danmu(ret, &msg) == -1) {
            send(sockfd, sendBuf, 0, 0);
        }
    }
    return 0;
}
